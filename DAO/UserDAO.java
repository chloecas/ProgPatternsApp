...
public class UserDAO {
    ...
    public boolean usernameAvailability(String user) throws Exception {
        try (Connection myCon = Database.getConnection();
             PreparedStatement avail = myCon.prepareStatement("SELECT username FROM Users WHERE username = ?")) {

            avail.setString(1, user);

            try(ResultSet result = avail.executeQuery()) {
                return result.next();
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    public boolean authenticate(String user, String pass) {
        try (Connection myCon = Database.getConnection();
            ...
            try(ResultSet result = auth.executeQuery()) {
                return result.next();
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
